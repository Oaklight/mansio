...
Verifies that PiazzaClient.note_write, thought_record, and memory_store
...
def _make_server(tmp_path):
    """Start a PiazzaServer with token auth, return (url, token_store, server)."""
...
class TestNoteWrite:
    ...
    def test_note_write_returns_message_id(self, tmp_path) -> None:
        ...
        try:
            ...
            client = PiazzaClient(url, "elena", token=token)
...
class TestServerStability:
    ...
    def test_multiple_agents_sugar_methods(self, tmp_path) -> None:
        ...
        try:
            ...
            alice = PiazzaClient(url, "alice", token=t1)
            bob = PiazzaClient(url, "bob", token=t2)
