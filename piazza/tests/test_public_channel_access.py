...
class TestPublicChannelQuery:
    ...
    def test_query_public_channel_shows_all_agents(self, auth_server) -> None:
        ...
        alice_t.publish("dev-piazza", "alice", "chat", "hello from alice")
        bob_t.publish("dev-piazza", "bob", "chat", "hello from bob")

        # Alice queries — should see both messages
        alice_msgs = alice_t.query("dev-piazza", limit=100)
        ...
        bob_msgs = bob_t.query("dev-piazza", limit=100)
...
class TestPublicChannelSSE:
    ...
    def test_subscribe_public_channel_no_403(self, auth_server) -> None:
        ...
        resp = http.get(f"{url}/v1/subscribe?channel=dev-piazza", stream=True)
