...
def _migrate_v2_to_v3(conn: sqlite3.Connection) -> None:
    """Rename agent_id → user_id in agent_presence and channel_acl tables."""
    # Rename agent_presence table → user_presence with column rename
    conn.executescript("""
        CREATE TABLE IF NOT EXISTS user_presence (
            user_id   TEXT PRIMARY KEY,
            last_seen TEXT NOT NULL,
        ...
        INSERT OR IGNORE INTO user_presence (user_id, last_seen, metadata)
            SELECT agent_id, last_seen, metadata FROM agent_presence;
        DROP TABLE IF EXISTS agent_presence;
    """)
    # Recreate channel_acl with user_id column
    conn.executescript("""
        CREATE TABLE IF NOT EXISTS _channel_acl_new (
            channel    TEXT NOT NULL REFERENCES channels(name) ON DELETE CASCADE,
            user_id    TEXT NOT NULL,
            permission TEXT NOT NULL DEFAULT 'write',
            granted_at TEXT NOT NULL,
        ...
        INSERT OR IGNORE INTO _channel_acl_new (channel, user_id, permission, granted_at, granted_by)
            SELECT channel, agent_id, permission, granted_at, granted_by FROM channel_acl;
        DROP TABLE IF EXISTS channel_acl;
        ALTER TABLE _channel_acl_new RENAME TO channel_acl;
    """)
