...
class TokenStore:
    ...
    Example:
        >>> store = TokenStore("/data/mansio.db")
    ...
    def _ensure_user_id_column(conn: sqlite3.Connection) -> None:
        """Migrate legacy agent_id column to user_id (idempotent)."""
    ...
    def validate(self, token_str: str) -> str | None | bool:
        ...
        matched_user: str | None | bool = False

        for row in rows:
            if secrets.compare_digest(provided_hash, row["token_hash"]):
                matched_id = row["id"]
                matched_user = row["user_id"]  # None for supertoken
                break
