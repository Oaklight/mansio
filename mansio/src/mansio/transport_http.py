...
from mansio.types import ACLEntry, ClaimResult, Message, UserPresence
...
class HttpTransport:
    ...
    def acl_set(self, channel: str, entries: list[ACLEntry]) -> int:
        ...
        """
        body = {"acl": [{"user_id": e.user_id, "permission": e.permission} for e in entries]}
        resp = self._http.put(
            f"{self._base_url}/v1/channels/{urllib.parse.quote(channel, safe='')}/acl",
            json=body,
        )
    ...
    def _report_callback_error(self, sub: _Subscription, msg: Message, exc: Exception) -> None:
