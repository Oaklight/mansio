...
from mansio.backends.nats import NATSBackend  # noqa: E402
from mansio.types import Message  # noqa: E402
...
def _make_backend(stream_suffix: str = "") -> NATSBackend:
    ...
    stream_name = f"MANSIO_TEST_{_RUN_ID}{stream_suffix}"
    return NATSBackend(
        url=NATS_URL,
        stream_name=stream_name,
        subject_prefix=f"mansio_test_{_RUN_ID}{stream_suffix}",
...
class TestNATSBackendChannelEncoding:
    ...
    def test_underscore_channel_roundtrip(self, backend: NATSBackend):
        ...
        msg = _make_msg(channel="dev_mansio")
        backend.store(msg)
        results = backend.query("dev_mansio")
        assert len(results) == 1
        assert results[0].channel == "dev_mansio"
